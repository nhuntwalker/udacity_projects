...
```{r}
ggplot(data=prosper, aes(x=BorrowerRate, y=LenderYield)) + geom_point() +
  xlab("Borrower Rate") +
  ylab("Lender Yield")
```
...
ggplot(data=prosper, aes(x=IsBorrowerHomeowner, y=LenderYield)) + geom_boxplot() +
  ...
  ylab("Lender Yield")
```
Hmm, now that's interesting. There's a slightly higher median yield from borrowers that DON'T own homes. I wonder why that is? 

I wonder if loan term length matters.
```{r}
ggplot(data=prosper, aes(x=IsBorrowerHomeowner, y=LenderYield)) + geom_boxplot() +
  xlab("Homeowner Status") +
  ylab("Lender Yield") +
  facet_wrap(~Term)
```
No matter the term length, those who don't own homes provide a larger Lender Yield!
...
The debt-to-income ratio vs mean credit score.
```{r}
...
The mean Credit Score vs Listing Category

```{r}
prosper_sub <- subset(prosper, (CreditScoreMean > 300) & (ListingCategory..numeric. != 0))

ggplot(data=prosper_sub, aes(x=factor(ListingCategory..numeric.), y=CreditScoreMean)) + geom_boxplot()
```
