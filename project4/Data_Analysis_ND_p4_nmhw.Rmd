...
```{r}
ggplot(data=subset(prosper, !(EmploymentStatus %in% c("", "Not available"))), aes(x=EmploymentStatus)) +
  geom_bar() +
...
```{r}
ggplot(data=subset(prosper, (StatedMonthlyIncome < 10000) & (StatedMonthlyIncome > 0) & !(EmploymentStatus %in% c("", "Not available"))),
       aes(y=LoanOriginalAmount, x=StatedMonthlyIncome)) +
...
```{r echo=FALSE}
ggplot(data=subset(prosper, CreditGrade != ''), aes(x=BorrowerAPR, y=LenderYield)) + 
...
What else influences lender yield?
```{r}
ggplot(data=prosper, aes(x=IsBorrowerHomeowner, y=LenderYield)) + geom_boxplot() +
  xlab("Homeowner Status") +
  ylab("Lender Yield")
```

Hmm, now that's interesting. There's a slightly higher median yield from borrowers that DON'T own homes. I wonder why that is?I wonder if loan term length matters.
```{r}
ggplot(data=prosper, aes(x=IsBorrowerHomeowner, y=LenderYield)) + geom_boxplot() +
  xlab("Homeowner Status") +
  ylab("Lender Yield") +
...
No matter the term length, those who don't own homes provide a larger Lender Yield! What about income range?
```{r}
ggplot(data=subset(prosper, !(IncomeRange %in% c("$0", "Not displayed"))), aes(x=IsBorrowerHomeowner, y=LenderYield)) + geom_boxplot() +
  xlab("Homeowner Status") +
  ylab("Lender Yield") +
...
produces a coefficient of 0.210. Even smaller when only considering loans that posted an actual loss (0.084).
...
```{r}
loss_bin = 500
ggplot(data=subset(prosper, LP_NetPrincipalLoss > 0), aes(x=round(LoanOriginalAmount / loss_bin) * loss_bin, y = LP_NetPrincipalLoss/LoanOriginalAmount)) + geom_point(alpha=0.1, position="jitter") +
  xlab("Loan Principal ($)") +
...
```{r echo=FALSE}
binsize = 1E3
ggplot(data=subset(prosper, !is.na(StatedMonthlyIncome) & (StatedMonthlyIncome > 0) & (StatedMonthlyIncome < 2.5E4)),
       aes(x=round(StatedMonthlyIncome / binsize) * binsize, y=TotalCreditLinespast7years)) +
...
ggplot(data=subset(prosper, !(EmploymentStatus %in% c("", "Not available"))), aes(x=EmploymentStatus)) +
  ...
  xlab("Employment Status") +
  ylab("Count") +
  scale_y_log10(breaks=c(1E2, 1E4, 1E6, 1E8, 1E10, 1E12)) +
  scale_fill_hue(name="Term (yrs)") +
  ggtitle("Employment Statuses of Loan Borrowers")
```

####Description:
The distribution of employment statuses across borrowers, split by the term length of the loans they borrow.
...
####Description:
Box plots of the lender yield vs homeowner status, split by income range.

####Plot Three
```{r echo=FALSE}
bw = 20
# prosper$rename_loan_buckets <- factor(prosper$LoanOriginalAmount.buckets,
#                                       levels=c("$1,000-3,000", "$3,000-5,000",
#                                                "$5,000-8,500", "$8,500-15,000",
#                                                "$15,000-35,000", "NA"))
ggplot(data=subset(prosper, !is.na(DebtToIncomeRatio) & !is.na(CreditScoreRangeUpper) & !is.na(CreditScoreRangeLower) & (CreditScoreRangeLower > 0) & (CreditScoreRangeUpper > 0) & (DebtToIncomeRatio < 1.25)), 
       aes(x=round(CreditScoreMean / bw) * bw, y=DebtToIncomeRatio)) +
...
####Description:
Scatter plots of the debt-to-income ratio of the borrower vs mean credit score, split by Loan Original Amount. Overplotted is the average debt-to-income ratio for each bin of average credit score, with bin width of 20.
